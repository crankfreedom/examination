...
export class ChalkLogin {
  ...
  async login(): Promise<void> {
    ...
    if (!haslogined) {
      ...
      await this.crawler.clickElement(
        "//span[contains(text(),'账号密码登录')]",
      );
      await this.crawler.sleep();
      await this.crawler.fillElement(
        'input.fenbi-login-modal-form-input[type="text"]',
        username,
        "css",
      );
      await this.crawler.fillElement(
        'input.fenbi-login-modal-form-input[type="password"]',
        password,
        "css",
      );
      await this.crawler.clickElement(
        ".fenbi-login-modal-agreement-checkbox",
        "css",
      );
  ...
  async isLogined(): Promise<boolean> {
    ...
    // 用cookie判断
    const userid = await this.crawler.getCookie("userid");
    return !!userid;
  }
}
